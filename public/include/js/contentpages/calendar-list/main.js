...
function changeTitleContentStatusText(){
    if(calendarType == 6){
        $("#ContentStatus").text("完成日期");
    }else{
        $("#ContentStatus").text("狀態");
    }
}
...
function putDataOption(putArea, pageStyleObj, content, putBefore){
    ...
    if(DesigneeID != PricipalID ){
        if(DesigneeID == userID && PricipalID != userID){
            ...
            if(content.MyKeypoint.Status.Uid == 2 || content.MyKeypoint.Status.Uid == 3){
                $(pageStyleObj).find(".fa-check").remove();
            }
        }
    }
    ...
    if(parseInt(content.MyKeypoint.Status.Uid) > 1){
        $(pageStyleObj).find(".fa-trash-o").remove();
    }
    ...
    var hasHistories = (content.Histories.length) ? true : false;
    if(Boolean(content.HasDetialItem) || content.MyKeypoint.Pricipal.Uid != userID || !hasHistories){
        finishBtn.remove();
    }
    // 事項標題
    ...
    var completeStr = content.MyKeypoint.Status.Name;

    if(parseInt(content.MyKeypoint.Status.Uid) == 2 ){
        if(!isDesignee){
            completeStr = content.Designee.Name;
            completeStr += "<br/>"+content.MyKeypoint.Status.Name; 
        }
    }

    if(calendarType == 6){
        completeStr = content.CompletionDate;
    }
    ...
    if(content.MyKeypoint.Status.Uid == 4 || content.MyKeypoint.Status.Uid == 12){
        $(pageStyleObj).find(".list-items").eq(3).addClass("received-back");
    }else{
        $(pageStyleObj).find(".list-items").eq(3).removeClass("received-back");
    }
    ...
    if(content.MyKeypoint.Status.Uid == 3){
    ...
    if(putArea != null && putArea != undefined){
        if(putArea.find(".data-empty").length){
            ...
            if(parseInt(content.MyKeypoint.Status.Uid) == 2){
                if($(putArea).find(".dataContent").length){
                    $(putArea).find(".dataContent").eq(0).before(pageStyleObj);
                }else{
                    $(pageStyleObj).appendTo(putArea);
                }
                return;
...
function creatHistory(historyData, historiesArea, isModify, modifyObj, isDataListItem, dataListItemID){
    ...
    if(historyData.length){
        ...
        getStyle(option,function(historyPage){
            $.each(historyData, function(historiesIndex, historiesContent){
                createHistories(historyPage, historiesContent, historiesArea, isModify, false, modifyObj, true, isDataListItem, dataListItemID);
            });
        });
...
// 新增辦況
function addHistory(content, putArea, isModify, isDesignee, modifyObj, isDataListItem, dataListItemID){
    if(isDataListItem == undefined){
        isDataListItem = false;
        dataListItemID = null;
    }
    ...
    getStyle(option,function(historyPage){
        ...
        createHistories(historyPage, content, putArea, true, isDesignee, modifyObj, true, isDataListItem, dataListItemID);
    });
}
...
function createDetailItem(detailStyle, content, putArea, isModify, isDesignee, isView, parentData, putBefore, isPut){
    ...
    if(content != null && content != undefined){
        if(parseInt(content.Type.Uid) == 1){
            $(detailObj).find("#addHistoryActBtn").remove();  
        }
    }
    ...
    //     }
    // }
    if(!isAssist && isDesignee){
        $(detailObj).find("#addHistoryActBtn").remove();
    }
    if(isDesignee || isAssist){
        // 加號，可新增歷程
        $(detailObj).find("#addHistoryActBtn").click(function(){
            $(this).removeClass("danger-font");
    ...
    if(content != null){
        if(!content.Designee.Uid == userID){
            $(detailObj).find(".actionBtnArea,.actionBtn").remove();
        }
    }
    ...
    if(isModify){
        if(isView){
            ...
            if((parentData.MyKeypoint.Status.Uid != 1 && parentData.MyKeypoint.Status.Uid != 4) || parentData.Type.Uid == 1){
                $(detailObj).find("#addHistoryActBtn").remove();
                $(detailObj).find(".fa-square-o").removeClass("send-btn mouse-pointer").addClass("ban-not");
            }else{
                ...
                if(content.Assist.Uid > 0 && content.Assist.Uid != userID){
                    $(detailObj).find(".fa-square-o").removeClass("send-btn mouse-pointer").addClass("ban-not");
                    $(detailObj).find("#addHistoryActBtn").parent().remove();
                    ...
                    $(detailObj).find(".fa-square-o").unbind('click').click(function(){
                        ...
                        if(isNotSave){
                            ...
                            var msg = "您還有尚未儲存的辦況，完成細項將不會存取未存檔之辦況，<br>";
                            msg += "您是否要不儲存辦況並完成細項？";
                            chooseDialog(msg, option);
                        }else{
                            // 完成事件
        ...
        if(content != null){
            ...
            if(content.Histories != undefined && content.Histories.length){
                ...
                if(!content.Histories.length){
                    $(detailObj).find("#addHistoryActBtn").addClass("danger-font");
                }
            }
            // 顯示協辦
    ...
    // 細項檔案上傳
    $(detailObj).find("#fileUploadBtn").unbind('click').click(function(){
        // return;
...
// 創建歷程
function createHistories(historiesStyle, content, putArea, isModify, isDesignee, modifyObj, isPut, isDataListItem, dataListItemID){
    if(isPut == undefined){
        isPut = true;
    }
    if(isPut){
    ...
    if(content == null){
        $(historiesObj).find(".fa-pencil-square-o").parent().hide();
        // $(historiesObj).find("#haveFile").hide();
    ...
    if(isModify){
        ...
        // 讀取、放檔案
        if(content.Doc != undefined && content.Doc.length){
            // $(historiesObj).find("#haveFile").show();
            if(!$(historiesObj).find("#haveFile").find(".fa-paperclip").length){
                var fileIcon = $("<i>").addClass("fa fa-paperclip mouse-pointer");
            ...
            if(fileIcon == undefined){
                var fileIcon = $(historiesObj).find("#haveFile").find(".fa-paperclip");
            }
            fileIcon.unbind("click").click(function(){
                var option = {
                    effect:"blind",
                    complete:function(){
                        if(!isExpand){
    ...
    $(historiesObj).find("input:text").unbind("keypress").keypress(function(keydown){
        if(keydown.which == 13) {
            ...
            if($.trim(Desc)){
                ...
                modifyHistoryBtn.parent().show();
                saveHistories(content, Desc, $(historiesObj), modifyObj, historiesForm, isNew, isDataListItem, dataListItemID);
            }else{
    ...
    // 檔案上傳
    $(historiesObj).find("#fileUploadBtn").unbind("click").click(function(){
        // 一維
        ...
        if(content != undefined){
            var option = {
              sureCall: function(){
                ...
                deleteHistoryData(modifyObj.Uid,content.Uid,removeArea, isDataListItem, dataListItemID);
              },
              sureText: "刪除",
              sureClass: "btn-danger",
              title: "刪除訊息"
    ...
    modifyBtn.unbind("click").click(function(){
        ...
        if($(this).prop("class").search("fa-pencil-square-o")!= -1){
            $(historiesObj).find(".fa-trash-o").hide();
            ...
            $(historiesObj).find("#historyFileInfo").find(".file-items").each(function(){
                ...
                trash.click(function(){
                    ...
                    var option = {
                      sureCall: function(){
                        deleteHistoryFile(content.Uid, fileUid, removeArea, historiesObj);
                      },
                      sureText: "刪除",
                      sureClass: "btn-danger",
                      title: "刪除訊息"
            ...
            historiesForm.empty();
            if($(historiesObj).find("#haveFile").find(".fa-paperclip").length){
                $(historiesObj).find("#haveFile").find(".fa-paperclip").show();
            }
        }
...
function deleteHistoryData(parentID,uid, modifyItem, isDataListItem, dataListItemID){
    // 
...
// 刪除檔案
function deleteHistoryFile(uid, fileID, modifyItem, historiesObj){
    var tmpData = {
            "history_id": uid,
            "doc_id": fileID
        };
    ...
    $.ajax({
        ...
        success: function(rs){
            ...
            if(!rs.Status){
                ...
                modifyItem.remove();
                if(!$(historiesObj).find("#historyFileInfo").find(".file-items").length){
                    $(historiesObj).find("#haveFile").find(".fa-paperclip").remove();
                }
            }
...
function checkViewHistory(){
  var isNotSave = false;
  $("#calendarViewDialog").find(".histories").find(".detail-item").each(function(){
      if($(this).find("#historyDesc").is(":visible")){
         isNotSave = true;
         return;
      }
  });
  $("#calendarViewDialog").find("#historiesArea").find(".detail-item").each(function(){
      if($(this).find("#historyDesc").is(":visible")){
         isNotSave = true;
         return;
      }
